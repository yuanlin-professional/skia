...
> 源文件: `tools/viewer/ChartSlide.cpp`

## 概述

ChartSlide 渲染 5 组带填充区域的滚动折线图数据，通过不断左移数据实现滚动动画效果。重点测试 GPU 路径填充性能（大量不规则多边形）。
...
ChartSlide : Slide - gen_data 生成随机数据点，gen_paths 生成描边和填充路径。填充区域使用半透明颜色，相邻图层共享底边。

# GlyphTransform - 表情符号变换动画

> 源文件: `tools/viewer/GlyphTransformSlide.cpp`

## 概述

GlyphTransformSlide 实现了一个灵感来自 d3.js 的表情符号动画效果，通过 PingPong 时间函数驱动文本的位移、缩放和旋转变换。
...
GlyphTransformView : Slide - 使用 TimeUtils::PingPong 生成振荡时间值，应用 sin/cos/sqrt 函数计算变换参数。

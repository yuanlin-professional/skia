...
Skia 现在提供了 WebAssembly 构建，以便在 Web 上轻松部署我们的图形 API。

CanvasKit 提供了一个用于测试新的 Canvas 和 SVG 平台 API 的试验场，
支持在 Web 平台上快速开发。它也可以作为需要尖端功能的自定义 Web 应用的
部署机制，例如 Skia 的 [Lottie 动画](https://skia.org/docs/user/modules/skottie)支持。

## 功能

- WebGL 上下文封装为 SkSurface，允许直接绘制到 HTML canvas
- 提供核心的 Skia canvas/paint/path/text API 集，详见 bindings
- 绘制到硬件加速后端
- 使用 Skia 的模糊测试器 (fuzzers) 进行安全测试
...
<div id=demo>
  ...
  <h3>使用 skottie 播放 bodymovin lottie 文件（点击查看 fiddle）</h3>
...
Lottie 文件由 lottiefiles.com 社区提供：
...
在我们的 [CanvasKit Fiddle](https://jsfiddle.skia.org/canvaskit) 上测试你的代码

## 下载

获取 [NPM 上的 CanvasKit](https://www.npmjs.com/package/canvaskit-wasm)。
文档和 Typescript 定义可在 npm 包的 `types/` 子文件夹中或从
[Skia 仓库](https://github.com/google/skia/tree/main/modules/canvaskit/npm_build/types)获取。

也请查看[快速入门指南](../quickstart)。
